"""FAN Courier parcel tracker custom component for Home Assistant."""
...
from .api import FANCourierApiClient
...
class FANCourierData:
    """Runtime data attached to the FAN Courier config entry."""

    client: FANCourierApiClient
    coordinator: FANCourierCoordinator


type FANCourierConfigEntry = ConfigEntry[FANCourierData]


async def async_setup_entry(hass: HomeAssistant, entry: FANCourierConfigEntry) -> bool:
    """Set up FAN Courier from a config entry."""
    # No auth: FAN Courier tracking is public, so the HA-managed session is fine.
    client = FANCourierApiClient(async_get_clientsession(hass))
    coordinator = FANCourierCoordinator(hass, client, entry)
    ...
    entry.runtime_data = FANCourierData(client=client, coordinator=coordinator)
...
async def _async_options_updated(
    hass: HomeAssistant, entry: FANCourierConfigEntry
) -> None:
    """Apply changed options by refreshing the coordinator."""
    await entry.runtime_data.coordinator.async_request_refresh()


async def async_unload_entry(hass: HomeAssistant, entry: FANCourierConfigEntry) -> bool:
    """Unload the FAN Courier config entry."""
