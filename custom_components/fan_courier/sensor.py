"""Sensor platform for the FAN Courier parcel tracker integration."""
...
from . import FANCourierConfigEntry
from .const import DOMAIN, ParcelStatus
from .coordinator import FANCourierCoordinator
...
async def async_setup_entry(
    hass: HomeAssistant,
    entry: FANCourierConfigEntry,
    async_add_entities: AddEntitiesCallback,
) -> None:
    """Set up FAN Courier sensor entities from a config entry."""
    ...
    entities: list[SensorEntity] = [
        FANCourierIncomingParcelsSensor(
            coordinator, entry, async_add_entities, current_barcodes
        ),
    ]
    for parcel in coordinator.data or []:
        entities.append(
            FANCourierParcelSensor(coordinator, entry, parcel.get("barcode", ""))
        )
    entities.append(FANCourierNextDeliverySensor(coordinator, entry))
    entities.append(FANCourierAwaitingPickupSensor(coordinator, entry))
    entities.append(FANCourierDeliveredParcelsSensor(coordinator, entry))
    entities.append(FANCourierLastUpdateSensor(coordinator, entry))

    async_add_entities(entities)


class FANCourierIncomingParcelsSensor(
    CoordinatorEntity[FANCourierCoordinator], SensorEntity
):
    ...
    def __init__(
        self,
        coordinator: FANCourierCoordinator,
        entry: ConfigEntry,
        async_add_entities: AddEntitiesCallback,
    ...
    def _handle_coordinator_update(self) -> None:
        ...
        if new_barcodes:
            self._async_add_entities(
                FANCourierParcelSensor(self.coordinator, self._entry, barcode)
...
class FANCourierParcelSensor(CoordinatorEntity[FANCourierCoordinator], SensorEntity):
    """Per-parcel sensor reporting the status of one tracked FAN Courier parcel."""
    ...
    def __init__(
        self, coordinator: FANCourierCoordinator, entry: ConfigEntry, barcode: str
    ) -> None:
        """Initialize the sensor."""
        super().__init__(coordinator)
        self._entry = entry
...
class FANCourierNextDeliverySensor(
    CoordinatorEntity[FANCourierCoordinator], SensorEntity
):
    ...
    def __init__(
        self, coordinator: FANCourierCoordinator, entry: ConfigEntry
    ) -> None:
        """Initialize the sensor."""
        super().__init__(coordinator)
...
class FANCourierAwaitingPickupSensor(
    CoordinatorEntity[FANCourierCoordinator], SensorEntity
):
    """Parcels that have arrived at a FAN Courier pickup point and are ready to collect."""
...
class FANCourierDeliveredParcelsSensor(
    CoordinatorEntity[FANCourierCoordinator], SensorEntity
):
    """Recently delivered tracked FAN Courier parcels."""
...
class FANCourierLastUpdateSensor(
    CoordinatorEntity[FANCourierCoordinator], SensorEntity
):
    """Diagnostic sensor reporting when FAN Courier was last polled successfully."""
