"""Config flow for the FAN Courier parcel tracker integration."""
...
def normalize_tracking_code(value: str) -> str:
    """Return a user-supplied code without changing its carrier semantics."""
    return (value or "").strip()


def valid_tracking_code(value: str) -> bool:
    """Accept any non-empty code; the carrier decides whether it exists."""
    return bool(value)
...
class FANCourierConfigFlow(ConfigFlow, domain=DOMAIN):
    """Handle the UI-driven configuration flow for the FAN Courier integration."""
    ...
    ) -> FANCourierOptionsFlowHandler:
        """Return the options flow handler."""
        return FANCourierOptionsFlowHandler()

    async def async_step_user(
        self, user_input: dict[str, Any] | None = None
    ) -> ConfigFlowResult:
        """Create the FAN Courier hub — single instance, no input needed.
        ...
        the ``fan_courier.track_parcel`` service or a dashboard button.
...
class FANCourierOptionsFlowHandler(OptionsFlow):
