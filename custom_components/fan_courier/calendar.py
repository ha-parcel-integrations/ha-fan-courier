"""Calendar platform for the FAN Courier parcel tracker integration."""
...
async def async_setup_entry(
    ...
    """Set up the FAN Courier deliveries calendar from a config entry."""
    async_add_entities([FANCourierDeliveriesCalendar(entry.runtime_data.coordinator, entry)])


class FANCourierDeliveriesCalendar(CoordinatorEntity[FANCourierCoordinator], CalendarEntity):
    """A read-only calendar of expected FAN Courier deliveries.
    ...
    def __init__(self, coordinator: FANCourierCoordinator, entry: ConfigEntry) -> None:
    ...
    def _events(self) -> list[CalendarEvent]:
        ...
        for parcel in self.coordinator.data or []:
            ...
            summary = sender or (f"Parcel {barcode}" if barcode else "FAN Courier parcel")
            description_parts = [
