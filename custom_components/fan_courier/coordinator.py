"""Coordinator for the FAN Courier parcel tracker integration.
...
from .api import FANCourierApiClient, FANCourierApiError
from .const import (
...
class FANCourierCoordinator(DataUpdateCoordinator[list[dict]]):
    ...
    async def _async_update_data(self) -> list[dict]:
        ...
        for code, result in zip(codes, results):
            if isinstance(result, BaseException):
                if not isinstance(
                    result, (FANCourierApiError, aiohttp.ClientError)
                ):
                    raise result
                errors += 1
                if (
                    isinstance(result, FANCourierApiError)
                    and result.status_code == 429
                ):
                    saw_429 = True
                    if result.retry_after is not None:
                        retry_afters.append(result.retry_after)
                _LOGGER.warning("FAN Courier fetch failed: %s", result)
                cached = self._raw_cache.get(code)
                if cached is not None:
                    raws.append((cached, code))
                continue
            ...
            if result is None:
                ...
                raws.append((self._raw_cache.get(code) or {}, code))
                continue

            self._raw_cache[code] = result
            raws.append((result, code))
        ...
        if saw_429:
            ...
            raise UpdateFailed(
                "FAN Courier rate-limited (429)", retry_after=retry_after
            )
        self._consecutive_429 = 0

        if codes and errors == len(codes) and not raws:
            raise UpdateFailed("FAN Courier unreachable for all tracked parcels")

        include_history = self._include_history
        normalized = [
            normalize_parcel(raw, tracking_code=code, include_history=include_history)
