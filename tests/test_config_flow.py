"""Tests for the FAN Courier config and options flow."""

from pytest_homeassistant_custom_component.common import MockConfigEntry

from custom_components.fan_courier.config_flow import (
...
async def test_user_flow_creates_hub_without_input(hass):
    ...
    assert result["title"] == "FAN Courier"
...
async def test_options_add_code_with_separators(hass):
    """Pasted codes retain their carrier-defined formatting."""
    entry = _hub([])
    entry.add_to_hass(hass)
    result = await _open_options_step(hass, entry, "parcels")
    result = await hass.config_entries.options.async_configure(
...
async def test_options_add_short_tracking_code(hass):
    entry = _hub([])
    entry.add_to_hass(hass)
    result = await _open_options_step(hass, entry, "parcels")
    result = await hass.config_entries.options.async_configure(
