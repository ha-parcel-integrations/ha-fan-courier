"""Tests for FAN Courier sensor property logic."""
from datetime import datetime, timezone
from unittest.mock import MagicMock

from custom_components.fan_courier.const import ParcelStatus
from custom_components.fan_courier.sensor import (
    FANCourierAwaitingPickupSensor,
    FANCourierDeliveredParcelsSensor,
    FANCourierIncomingParcelsSensor,
    FANCourierLastUpdateSensor,
    FANCourierNextDeliverySensor,
    FANCourierParcelSensor,
)
...
def test_incoming_counts_and_lists():
    ...
    sensor = FANCourierIncomingParcelsSensor(coordinator, _entry(), lambda _: None, set())
...
def test_parcel_sensor_status_and_attributes():
    ...
    sensor = FANCourierParcelSensor(_coordinator([parcel]), _entry(), "A")
...
def test_parcel_sensor_missing_barcode():
    sensor = FANCourierParcelSensor(_coordinator([_parcel("A")]), _entry(), "OTHER")
...
def test_next_delivery_picks_earliest():
    ...
    sensor = FANCourierNextDeliverySensor(coordinator, _entry())
...
def test_next_delivery_none_without_moments():
    sensor = FANCourierNextDeliverySensor(_coordinator([_parcel("A")]), _entry())
    assert sensor.native_value is None
    assert sensor.extra_state_attributes == {}
...
def test_awaiting_pickup_counts_only_pickup_ready_parcels():
    coordinator = _coordinator([
        _parcel("A", status=ParcelStatus.AT_PICKUP_POINT, pickup=True),
        _parcel("B", status=ParcelStatus.IN_TRANSIT, pickup=False),
        _parcel("C", status=ParcelStatus.DELIVERED, pickup=True),
    ])
    sensor = FANCourierAwaitingPickupSensor(coordinator, _entry())
...
def test_delivered_sensor():
    ...
    sensor = FANCourierDeliveredParcelsSensor(coordinator, _entry())
    assert sensor.native_value == 1
...
def test_last_update_sensor():
    ...
    sensor = FANCourierLastUpdateSensor(coordinator, _entry())
...
def test_awaiting_pickup_counts_rerouted_parcel_without_pickup_flag():
    ...
    sensor = FANCourierAwaitingPickupSensor(_coordinator([rerouted, _parcel("HOME")]), _entry())
    assert sensor.native_value == 1
