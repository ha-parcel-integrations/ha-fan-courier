"""Tests for FAN Courier setup and unload."""
...
from custom_components.fan_courier.api import FANCourierApiError
...
async def test_setup_and_unload(hass):
    ...
    with patch(
        "custom_components.fan_courier.api.FANCourierApiClient.async_get_parcel",
    ...
    incoming = hass.states.get("sensor.fan_courier_incoming_parcels")
...
async def test_setup_retries_when_first_refresh_fails(hass):
    ...
    with patch(
        "custom_components.fan_courier.api.FANCourierApiClient.async_get_parcel",
        new=AsyncMock(side_effect=FANCourierApiError("FAN Courier unreachable")),
    ):
...
async def test_response_awb_does_not_replace_user_tracked_code(hass):
    entry = MockConfigEntry(
        domain=DOMAIN,
        unique_id=DOMAIN,
        options={CONF_PARCELS: [{CONF_TRACKING_CODE: ACTIVE_CODE}]},
    ...
    with patch("custom_components.fan_courier.api.FANCourierApiClient.async_get_parcel", new=mock):
        assert await hass.config_entries.async_setup(entry.entry_id)
        await hass.async_block_till_done()
        ...
        # The endpoint may echo a different identifier; the user-supplied code
        # remains the sensor identity.
        ...
        assert registry.async_get_entity_id("sensor", DOMAIN, f"{entry.entry_id}_{OTHER_CODE}") is None
