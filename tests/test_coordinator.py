"""Tests for the FAN Courier coordinator: fetching, caching and events.
...
from custom_components.fan_courier.coordinator import FANCourierCoordinator
...
def _in_transit(code: str = ACTIVE_CODE) -> dict:
    ...
    sample["events"][-1]["id"] = "H0"
    sample["events"][-1]["name"] = "In transit"
    return sample
...
async def test_update_keeps_cached_payload_on_error(hass):
    ...
    client.async_get_parcel.side_effect = FANCourierApiError("HTTP 500")
...
async def test_fires_delivery_time_changed_event(hass):
    ...
    assert events == []  # FAN exposes no confirmed delivery window
